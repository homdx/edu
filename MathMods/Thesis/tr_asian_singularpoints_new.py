...
def show_results() -> None:
	...
	eu = vanilla_call(s0=s0, sigma=sigma, q=q, k=k, t=t, n=n, amer=False)
	# sp = sp_asian_call(s0=s0, sigma=sigma, q=q, k=k, t=t, am=True, n=n)
	...
	# print('Price of the call = {prc}'.format(prc=sp[0][0][1]))
...
def run(ns: list, d: int=6) -> None:
	...
	for n in ns:
		prc_ub = sp_asian_call(s0=s0, sigma=sigma, q=q, k=k, t=t, am=False, n=n, h=h, ub=True)[0][0][1]
		prc_lb = sp_asian_call(s0=s0, sigma=sigma, q=q, k=k, t=t, am=False, n=n, h=h, ub=False)[0][0][1]
		if n < 25:
			prc_ac = sp_asian_call(s0=s0, sigma=sigma, q=q, k=k, t=t, am=True, n=n)[0][0][1]
			print('n={n:3}: {lb:.{d}f} <= {ac:.{d}f} <= {ub:.{d}f}'.format(d=d, n=n, lb=prc_lb, ac=prc_ac, ub=prc_ub))
		else:
			print('n={n:3}: {lb:.{d}f} <= {ub:.{d}f}'.format(d=d, n=n, lb=prc_lb, ub=prc_ub))
