...
The following example, copied verbatim from Investopedia\footnote{http://www.investopedia.com/terms/c/cliquet.asp}, provides a good illustration.
\begin{eg}
...
\begin{rem} \label{rem:sp-clq-char}
...
The previous argument may be applied iteratively in a backward fashion at each step $ N-2, N-3, \dots, 1, 0 $ to obtain the singular points at each time step. At time 0, there is only one singular point $ (0, P_0^1) $, and $ P_0^1 $ provides the exact binomial price of the cliquet option. The equality of this price and the exact binomial price is proved in Proposition \ref{thm:sp-clq-equality}.
...
\begin{proof}
	Let the running sum $ Z_{i_1, \dots, i_k} = \sum_{j=1}^{k} R_{i_j}', \  (i_1, \dots, i_k) \in [j_0]^k, k \in \{ 1, \dots, N \} $.
...
The following proposition proves that the approximation of the price obtained by this method does converge to the price of the cliquet option in the continuous model.
...
\begin{enumerate}
	...
	\item Varying volatility for each observational period, as $ \sigma(i) = 0.05 + 0.04i, i = 1, \dots, 8 $ (denoted by \emph{Var} in the $ \sigma $ column of Table \ref{tab:sp-clq-results}).
\end{enumerate}
...
The specifications of the cliquet contract are as follows.
\begin{itemize}
...
[TODO: Insert remark 3]



\begin{table}[h]
	\label{tab:sp-clq-results}
	\centering
	\caption{Results}
