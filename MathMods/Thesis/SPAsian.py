...
def sp_asian_e_call(s0: float, k: float, sigma: float, q: float, n: int, t: float) -> list:
	...
	for i in range(n-1, -1, -1):
		...
		for j in range(i+1):
			...
			# TODO: Remove this section. DEBUG: Raise an ArithmeticError for empty singular value lists.
			if len(sp[i][j]) == 0:
				print("ERROR: Number of singular points must be positive.")
				print("DEBUG: N({i},{j}): \n\tSP({ipp},{j})={n_ipp_j} \n\tSP({ipp},{jpp})={n_ipp_jpp}.".
				      format(i=i, j=j, ipp=i+1, jpp=j+1, n_ipp_j=sp[i+1][j], n_ipp_jpp=sp[i+1][j+1]))
...
def run_test(s0: float, k: float, sigma: float, q: float, n: int, t: float, d: int):
	...
	for i in range(n+1):
		for j in range(i+1):
			...
			for (l, pt) in enumerate(eac[i][j]):
				eac[i][j][l] = (round(eac[i][j][l][0], d), round(eac[i][j][l][1], d))
...
def run_all_tests(ns: range):
	for i in ns:
		try:
			eac = sp_asian_e_call(s0=s0, k=k, sigma=sigma, q=q, n=i, t=t)
			print("Success: n={n:2}, price={prc:0.6f}".format(n=i, prc=eac[0][0][0][1]))
		except ArithmeticError:
			print("Fail: n={n:2}".format(n=i))


# Display the results for n=4
run_test(s0=s0, k=k, sigma=sigma, q=q, n=4, t=t, d=6)

# Display the results for n in 1:25
run_all_tests(range(1, 26))
