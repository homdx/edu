...
def crr(n, t, s0, r, sigma):
        ...
        for j in range (0, n+1):
                eu[n-i+2*j][i] = sqrt(max(s[n-i+2*j][i] - 324, 0)) * (s[n-i+2*j][i] < 529)
                am[n-i+2*j][i] = sqrt(max(s[n-i+2*j][i] - 324, 0)) * (s[n-i+2*j][i] < 529)
        for i in range(n-1, -1, -1):
                for j in range (0, i+1):
                        eu[n-i+2*j][i] = (p * eu[n-i+2*j-1][i+1] + (1-p) * eu[n-i+2*j+1][i+1]) / r
                        am[n-i+2*j][i] = max((p * eu[n-i+2*j-1][i+1] + (1-p) * eu[n-i+2*j+1][i+1]) / r,
                                             sqrt(max(s[n-i+2*j][i] - 324, 0)) * (s[n-i+2*j][i] < 529))
